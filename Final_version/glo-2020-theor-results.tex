...
Under \hypref{assum:strong:mixing} and \hypref{assum:bias:bound} we may establish the next theorem, which is the main result of this section. The proof is postponed to Section~\ref{sec:proofs} in the supplement.  
...
$$
and $\sup_{\theta \in \Theta} \int |\nabla_\theta \ud{n ; \theta}(x_n, x_{n + 1})| \, \mu_{n + 1}(\rmd x_{n + 1}) \leq c < \infty$ for some positive constant $c$. Then \hypref{assum:bias:bound} holds in the sense that for all $n \in \nset$, $h \in \bmf{\Xfd_{n + 1}}$, and $x_n \in \Xset$, 
$$
